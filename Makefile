...
.PHONY: help setup env run ui eval docker-build docker-run docker-up clean
...
help:
	...
	@echo   make docker-up     - Start Web UI at http://localhost:8000 (Docker)
	...
	@echo   make ui            - Start Web UI locally (http://localhost:8000)
...
docker-run:
	docker run -it --rm --env-file .env -v .:/app multi-agent-researcher:latest python main.py
